...
const baseProfiles = [
  { id:'developer', name:'Max Developer', email:'developer@steelcraft.local', portals: portals.map(([id]) => id) },
...
function DeveloperRoom({enabledPortals,setEnabledPortals,setActive}) {
  ...
  const rows = [
    ['Brand Room','Open the private developer-only Brand Room for logo, UI/UX style, color, and tenant look.','Private'],
  ...
  return <>
    ...
    <div className="workspace-grid">
      <article className="feature panel large"><p className="eyebrow">Max developer controls</p><h2>System builder room</h2><p>This is where the powerful controls live. Brand Room and portal enablement are developer-only. Customer admins never see these controls.</p><div className="quote-actions"><button type="button" onClick={() => { unlockBrandRoom(); window.location.href = '/brand'; }}>Open Brand Room</button><button type="button" onClick={() => setActive('admin')}>Preview Admin Portal</button></div></article>
...
function App(){
  ...
  if(!user) return <Auth brand={brand} profiles={profiles} onSignIn={p=>{setUserId(p.id); if (p.id !== 'developer') lockBrandRoom(); const first=p.portals.find((id)=>id==='developer'||enabledPortals.includes(id)) || 'admin'; setActive(first);}}/>;
  ...
  return <Shell brand={brand} user={user} active={safe} setActive={setActive} signOut={()=>{ lockBrandRoom(); setUserId(null); }} enabledPortals={enabledPortals}>{safe==='developer'?<DeveloperRoom enabledPortals={enabledPortals} setEnabledPortals={setEnabledPortals} setActive={setActive}/>:safe==='admin'?<Admin profiles={profiles} setProfiles={setProfiles} enabledPortals={enabledPortals}/>:safe==='accounting'?<AccountingPortal/>:<Portal id={safe}/>}</Shell>;
}
