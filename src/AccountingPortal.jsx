...
const accountingSections = [
  ...
  ['setup', 'Setup', 'Customers, vendors, accounts, and accounting controls']
];
...
function Field({ label, children }) { return <label className="brand-field"><span>{label}</span>{children}</label>; }
...
function CustomerForm({ submitAction, message }) {
  ...
  return <FormCard title="Add customer" description="Only add what you need. You can finish details later." onSubmit={submit} submitLabel="Add customer" busy={message.busy}><Field label="Customer name"><input value={form.customerName} onChange={update('customerName')} required /></Field><Field label="Contact"><input value={form.contactName} onChange={update('contactName')} /></Field><Field label="Email"><input type="email" value={form.email} onChange={update('email')} /></Field><Field label="Phone"><input value={form.phone} onChange={update('phone')} /></Field><Field label="Terms"><select value={form.terms} onChange={update('terms')}><option>Due on receipt</option><option>Net 15</option><option>Net 30</option><option>Net 45</option></select></Field></FormCard>;
}
...
function VendorForm({ submitAction, message }) {
  ...
  return <FormCard title="Add vendor" description="Add a vendor for bills, debit card charges, checks, and payments." onSubmit={submit} submitLabel="Add vendor" busy={message.busy}><Field label="Vendor name"><input value={form.vendorName} onChange={update('vendorName')} required /></Field><Field label="Contact"><input value={form.contactName} onChange={update('contactName')} /></Field><Field label="Email"><input type="email" value={form.email} onChange={update('email')} /></Field><Field label="Phone"><input value={form.phone} onChange={update('phone')} /></Field><Field label="Terms"><select value={form.terms} onChange={update('terms')}><option>Due on receipt</option><option>Net 15</option><option>Net 30</option><option>Net 45</option></select></Field></FormCard>;
}

function InvoiceForm({ customers, submitAction, message }) {
  ...
  const update = (key) => (event) => setForm((current) => ({ ...current, [key]: event.target.value }));
  async function submit(event) { event.preventDefault(); const subtotal = numberOrNull(form.subtotal) || 0; const tax = numberOrNull(form.tax) || 0; const retainage = numberOrNull(form.retainage) || 0; await submitAction('/api/accounting/invoices', { ...form, customerId: numberOrNull(form.customerId), subtotal, tax, retainage, total: subtotal + tax - retainage }, 'Invoice created.'); setForm({ customerId: '', invoiceNumber: '', invoiceType: 'progress', status: 'sent', issueDate: today(), dueDate: dueIn(30), subtotal: '', tax: '0', retainage: '0', notes: '' }); }
  return <FormCard title="Create invoice" description="Create the invoice. Neroa can handle the proof trail in the background." onSubmit={submit} submitLabel="Create invoice" busy={message.busy}><Field label="Customer"><select value={form.customerId} onChange={update('customerId')}><option value="">Select customer</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.customer_name}</option>)}</select></Field><Field label="Invoice number"><input value={form.invoiceNumber} onChange={update('invoiceNumber')} placeholder="Auto if blank" /></Field><Field label="Invoice type"><select value={form.invoiceType} onChange={update('invoiceType')}><option value="progress">Progress</option><option value="deposit">Deposit</option><option value="final">Final</option><option value="change_order">Change order</option><option value="subscription">Subscription</option></select></Field><Field label="Due date"><input type="date" value={form.dueDate} onChange={update('dueDate')} /></Field><Field label="Amount"><input type="number" step="0.01" value={form.subtotal} onChange={update('subtotal')} required /></Field><Field label="Tax"><input type="number" step="0.01" value={form.tax} onChange={update('tax')} /></Field><Field label="Retainage"><input type="number" step="0.01" value={form.retainage} onChange={update('retainage')} /></Field><Field label="Notes"><textarea value={form.notes} onChange={update('notes')} /></Field></FormCard>;
}
...
function BillForm({ vendors, submitAction, message }) {
  ...
  return <FormCard title="Enter bill" description="Enter what came in. AI bookkeeping can help route it later." onSubmit={submit} submitLabel="Save bill" busy={message.busy}><Field label="Vendor"><select value={form.vendorId} onChange={update('vendorId')}><option value="">Select vendor</option>{vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.vendor_name}</option>)}</select></Field><Field label="Bill number"><input value={form.billNumber} onChange={update('billNumber')} placeholder="Auto if blank" /></Field><Field label="PO number"><input value={form.poNumber} onChange={update('poNumber')} /></Field><Field label="Due date"><input type="date" value={form.dueDate} onChange={update('dueDate')} /></Field><Field label="Amount"><input type="number" step="0.01" value={form.subtotal} onChange={update('subtotal')} required /></Field><Field label="Tax"><input type="number" step="0.01" value={form.tax} onChange={update('tax')} /></Field><Field label="Notes"><textarea value={form.notes} onChange={update('notes')} /></Field></FormCard>;
}
...
function MoneyInView({ customers, invoices, submitAction, message, openSection }) { return <section className="accounting-focus-grid"><article className="feature panel large"><h2>Money In</h2><p>Simple question: who owes us, what needs invoiced, and who paid?</p><div className="accounting-actions-list"><button onClick={() => openSection('banking')}>Review deposits</button><button onClick={() => openSection('reports')}>AR report</button></div></article><InvoiceForm customers={customers} submitAction={submitAction} message={message} /><CustomerForm submitAction={submitAction} message={message} /><TableCard title="Customer invoices" rows={invoices} emptyLabel="No invoices yet." /></section>; }
...
function SectionView(props) {
  ...
  if (activeSection === 'money-in') return <MoneyInView customers={customers} invoices={invoices} submitAction={submitAction} message={message} openSection={openSection} />;
  ...
  return <SetupView customers={customers} vendors={vendors} accounts={accounts} submitAction={submitAction} message={message} status={status} />;
}
